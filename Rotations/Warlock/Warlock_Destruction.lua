...
local function autoTarget()
    ...
    if target:Debuff(debuffs.havoc, true) and target:DebuffRemains(debuffs.wither, true) + target:DebuffRemains(debuffs.immolate, true) < 5000 and gs.dotCount == gs.activeEnemies - 1 then
        return false
    end

    if A.GetToggle(2, "swapOffHavoc") and not player:TalentKnown(Mayhem.id) and target:Debuff(debuffs.havoc, true) and gs.activeEnemies >= 2 then
        -- Swap off Havoc only once per Havoc window, and only if a valid non-Havoc target exists
        if not gs.havocSwapDoneThisWindow then
            local foundNonHavoc = false
            local plates = MultiUnits:GetActiveUnitPlates()
            for enemyGUID in pairs(plates) do
                local enemy = MakUnit:new(enemyGUID)
                if enemy and Incinerate:InRange(enemy) and (enemy.canAttack or enemy.isMelee) and not enemy:Debuff(debuffs.havoc, true) then
                    foundNonHavoc = true
                    break
                end
            end
            if foundNonHavoc then
                gs.havocSwapDoneThisWindow = true
                return true
            end
        end
    end
...
local function updategs()
    ...
    if not gs.havocUp then gs.havocSwapDoneThisWindow = false end
...
Immolate:Callback("arena", function(spell, enemy, icon)
        if spell:IsImmune(enemy) or enemy:IsTotalImmune() or enemy:IsMagicImmune() then return end
        if enemy:HasBuff(witherMagicImmune) then return end
        if not isValidTarget(enemy) then return end
        if enemy.Los and not enemy:Los() then return end
        if enemy.cc and enemy:CCRemains() > 0 then return end
...
-- Extra magic immunity buffs for Wither
local witherMagicImmune = {
