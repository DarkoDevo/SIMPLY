...
local ActionID = {
    ...
    JadefireStomp            = { Type = "Spell", ID = 388193 },
    ...
    SphereHelp              = { Type = "Spell", ID = 410777, MAKULU_INFO = { heal = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
    SphereHarm              = { Type = "Spell", ID = 410777, Texture = 274738, Hidden = true, MAKULU_INFO = { damageType = "magic" }, Macro = "/cast [@target,harm][@focus,harm]spell:thisID" },
    ZenSpheres              = { Type = "Spell", ID = 410777, MAKULU_INFO = { heal = true, damageType = "magic" }, Macro = "/cast [@target,help][@focus,help][@target,harm][@focus,harm]spell:thisID" },
    ...
    LifeCocoon             = { Type = "Spell", ID = 116849, MAKULU_INFO = { heal = true, offGcd = true, ignoreCasting = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
    Vivify                  = { Type = "Spell", ID = 116670, MAKULU_INFO = { heal = true, ignoreCasting = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
    SoothingMist            = { Type = "Spell", ID = 115175, MAKULU_INFO = { heal = true, ignoreCasting = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
    ...
    EnvelopingMist          = { Type = "Spell", ID = 124682, MAKULU_INFO = { heal = true, ignoreCasting = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
    ...
    SheilunsGift            = { Type = "Spell", ID = 399491, MAKULU_INFO = { heal = true, ignoreCasting = true }, Macro = "/cast [@target,help][@focus,help]spell:thisID" },
...
-- NEW: Enhanced cooldown management function
local function enhancedBurstMode()
    local burstMode = A.GetToggle(2, "burstMode")
    if not burstMode then
        return makBurst() -- Use default burst logic
    end

    -- Enhanced burst mode logic
    local burstThreshold = A.GetToggle(2, "burstThreshold")

    -- Check for incoming damage within threshold
    if incBigDmgIn() > 0 and incBigDmgIn() < (burstThreshold * 1000) then
        return true
    end

    -- Check for group damage requiring burst healing
    local below50 = MakMulti.party:Count(function(friendly) return friendly.hp > 0 and friendly.ehp < 50 end)
    if below50 >= 2 then
        return true
    end

    -- Default to standard burst logic
    return makBurst()
end

local function useCelestialConduit()
    -- NEW: Check hero talent build selection
    local heroTalentBuild = A.GetToggle(2, "heroTalentBuild")
    if heroTalentBuild ~= 1 then return false end -- Only for Conduit build
    ...
    -- NEW: Use interface control for group threshold
    local groupThreshold = A.GetToggle(2, "celestialConduitGroupThreshold")
    ...
    -- Use configurable group threshold
    if below70 >= groupThreshold or below60 >= (groupThreshold - 1) or (below40 >= 1 and LifeCocoon.cd > 400) then
        return true
    end
end
...
local function useCocoon(unit)
    ...
    -- NEW: Emergency HP threshold - highest priority
    local emergencyHP = A.GetToggle(2, "emergencyHP")
    if unit.hp > 0 and unit.hp < emergencyHP then
        return true
    end
...
local function updategs()
    ...
    -- Season 3: Cache tier set status for performance
    local tierStatus = getTierSetStatus()
    gs.has2SetBonus = tierStatus.has2Set
    gs.has4SetBonus = tierStatus.has4Set
    gs.heartOfJadeActive = tierStatus.heartOfJadeActive
    gs.heartOfJadeRemains = tierStatus.heartOfJadeRemains
    gs.potentialEnergyStacks = tierStatus.potentialEnergyStacks
    gs.jadeSerpentBlessingActive = tierStatus.jadeSerpentBlessingActive
    
    local name, instanceType, difficultyID, difficultyName, maxPlayers, dynamicDifficulty, isDynamic, instanceID, instanceGroupSize, LfgDungeonID = GetInstanceInfo()
    gs.dungeon = instanceType == "party"
    gs.raid = instanceType == "raid"

    -- Hero Talent Detection with Interface Override
    local heroTalentBuild = A.GetToggle(2, "heroTalentBuild")
    if heroTalentBuild == 1 then -- Conduit
        gs.conduitOfCelestials = true
        gs.masterOfHarmony = false
    elseif heroTalentBuild == 2 then -- Harmony
        gs.masterOfHarmony = true
        gs.conduitOfCelestials = false
    else
        -- Auto-detect if not manually set
        gs.masterOfHarmony = player:TalentKnown(AspectOfHarmony.id)
        gs.conduitOfCelestials = not gs.masterOfHarmony -- Opposite hero talent tree
    end

    -- Enhanced Fistweaving System with Intensity Controls
    gs.fistweaving = A.GetToggle(2, "Fistweaving") or false
    gs.fistweavingMode = A.GetToggle(2, "fistweavingMode") or 2 -- Default: Balanced
    gs.pureFistInRaids = A.GetToggle(2, "pureFistInRaids") or false

    -- Determine fistweaving intensity based on mode and environment
    local canFistweave = gs.masterOfHarmony or player:TalentKnown(AncientTeachings.id)
    if gs.fistweavingMode == 1 then -- Pure
        gs.shouldFistweave = gs.fistweaving and canFistweave and (not gs.raid or gs.pureFistInRaids)
    elseif gs.fistweavingMode == 2 then -- Balanced
        gs.shouldFistweave = gs.fistweaving and canFistweave
    else -- Minimal (3)
        gs.shouldFistweave = gs.fistweaving and canFistweave and gs.actualLowest.hp > 80
    end
...
ManaTea:Callback(function(spell, unit)
    local stacks = player:HasBuffCount(buffs.manaTea)
    local pctRecovered = stacks * 2.5
    ...
    -- NEW: Stack-based logic - Use at ≥10 stacks OR ≥18 stacks for emergency
    if stacks >= 18 then
        -- Emergency usage at 18+ stacks
        return spell:Cast()
    end

    if stacks >= 10 then
        -- Standard usage at 10+ stacks when safe
        if unit.hp > 80 and player.manaPct < 80 then
            return spell:Cast()
        end
    end

    -- Emergency low mana usage
...
-- Critical Vivify - NEW: Emergency situations (highest priority after Life Cocoon)
Vivify:Callback("critical", function(spell, unit)
    if unit.ehp > A.GetToggle(2, "criticalHP") then return end

    -- Highest priority after Life Cocoon - emergency healing
    return spell:Cast(unit)
end)
...
JadefireStomp:Callback("core", function(spell)
    ...
    if player:TalentKnown(AugustDynasty.id) and not player:Buff(buffs.augustDynasty) and useAugustCombo() then
        return spell:Cast()
    end
end)
...
TigerPalm:Callback("mystic-touch", function(spell)
    if not target.exists or target.isFriendly then return end

    -- Apply Mystic Touch for enhanced fistweaving
    if not target:Debuff(buffs.mysticTouch) then
        return spell:Cast(target)
    end

    -- Maintain Mystic Touch if expiring soon
    if target:DebuffRemains(buffs.mysticTouch) < 3000 then
        return spell:Cast(target)
    end
end)
...
SummonJadeSerpentStatue:Callback(function(spell)
    if not player.combat and target.exists and not target.isFriendly and A.Paralysis:InRange(target) and A.SummonJadeSerpentStatue.used < 4000 then
        return spell:Cast()
    end

    if player.combat and (spell.used > player.combatTime * 1000 or spell.used < 0) then
        return spell:Cast()
    end
end)
...
local function fistweavingRotation(unit)
    ...
    -- Enhanced Fistweaving: Damage abilities for healing through Ancient Teachings/Jadefire Teachings
...
local function healing(unit)
    LifeCocoon("core", unit)
    -- NEW: Critical Vivify - highest priority after Life Cocoon
    Vivify("critical", unit)
    Detox("core", unit)
    ThunderFocusTea("harmony", unit)
    ThunderFocusTea("core")
