...
local function shouldBurst()
    if A.BurstIsON("target") then
        -- In arena or battlegrounds, always allow burst when toggled on
        if A.IsInPvP then
            return true
        end
        -- In PvE, check if enemies have enough health to warrant cooldowns
...
TouchOfDeath:Callback(function(spell)
    ...
    if target.physicalImmune then return end

    return spell:Cast(target)
end)

Vivify:Callback(function(spell)
    -- Only cast if Vivacious Vivification buff is active
    ...
    -- Heal current target if friendly and below 75% HP
    if target:Exists() and target:IsFriendly() and target.hp <= 75 and spell:InRange(target) then
        return spell:Cast(target)
    end
end)
...
WeaponsOfOrder:Callback(function(spell)
    ...
    -- PvP: require InvokeNiuzao buff to be active
    if A.IsInPvP then
        if not player:Buff(132578) then return end -- Invoke Niuzao buff
        return spell:Cast(player)
    end

    -- PvE: require enemies in melee
...
InvokeNiuzao:Callback(function(spell)
    -- PvP: cast with WeaponsOfOrder for burst (after 3s in combat)
    if A.IsInPvP and shouldBurst() and WeaponsOfOrder.cd == 0 and player:CombatTime() >= 3 then
        return spell:Cast(player)
    end

    -- PvE: only when Heavy stagger
